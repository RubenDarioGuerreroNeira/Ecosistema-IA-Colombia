...
export class AntioquiaHealthService implements OnModuleInit {
  ...
  async loadData() {
    try {
      ...
      // Build index by normalized municipio
      this.providersByMunicipio.clear();
      for (const provider of this.providers) {
        if (provider.municipio) {
          const norm = normalizeString(provider.municipio);
          if (norm) {
            if (!this.providersByMunicipio.has(norm)) {
              this.providersByMunicipio.set(norm, []);
            }
            this.providersByMunicipio.get(norm)!.push(provider);
          }
        }
      }
    } catch (error) {
  ...
  /** Returns unique list of municipios */
  getMunicipios(): string[] {
    const municipios = this.providers
      .map(p => p.municipio || '')
      .filter(m => m && m.trim().length > 0)
      .map(m => m.toString());
    return Array.from(new Set(municipios.map(m => normalizeString(m)))).filter(t => t.length >= 3 && !stopwords.has(t));
  }

  /** Search providers by normalized query */
  searchProviders(query: string): AntioquiaHealthProvider[] {
    ...
    // Department token yields all providers
    ...
    return this.providers.filter(p => {
      ...
      return tokens.some(tok => fields.some(fld => fld.includes(tok) || tok.includes(fld)));
    });
  }
